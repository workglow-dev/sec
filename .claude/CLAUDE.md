...
| `SEC_EMBEDDING_MODEL` | Embedding model — changing it invalidates the index. Its output width must be one this project records, or `SEC_EMBEDDING_DIMENSIONS` must state it |
| `SEC_EMBEDDING_DIMENSIONS` | The model's output width, for a model whose width is not recorded. The vector column is created at it |
